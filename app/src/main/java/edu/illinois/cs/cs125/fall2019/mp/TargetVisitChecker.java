...
public class TargetVisitChecker {
    ...
    public static boolean checkSnakeRule(final double[] latitudes, final double[] longitudes, final int[] path,
        ...
        for (int i = 0; i < last - 2; i++) {
            com.google.android.gms.maps.model.LatLng first = new
                    LatLng(latitudes[path[i]], longitudes[path[i]]);
            com.google.android.gms.maps.model.LatLng second = new
                    LatLng(latitudes[path[i + 1]], longitudes[path[i + 1]]);
            com.google.android.gms.maps.model.LatLng third = new
                    LatLng(latitudes[path[last - 1]], longitudes[path[last - 1]]);
            com.google.android.gms.maps.model.LatLng fourth = new
                    LatLng(latitudes[tryVisit], longitudes[tryVisit]);
            if (LineCrossDetector.linesCross(first, second, third, fourth)) {
                return false;
            }
            /*
            if (LineCrossDetector.linesCross(latitudes[path[i]], longitudes[path[i]],
                    latitudes[path[i + 1]], longitudes[path[i + 1]],
                    latitudes[path[last - 1]], longitudes[path[last - 1]],
