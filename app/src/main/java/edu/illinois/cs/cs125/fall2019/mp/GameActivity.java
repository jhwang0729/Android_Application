...
public final class GameActivity extends AppCompatActivity {
    ...
    /**
     * AreaDivider AreaDivider.
     */
    private AreaDivider areaDivider1;
    /**
     * Intent AreaDivider.
     */
    private Intent intent1;
    /**
     * visitedX AreaDivider.
     */
    ...
    protected void onCreate(final Bundle savedInstanceState) {
        ...
        intent1 = getIntent();
        if (intent1.getStringExtra("mode").equals("target")) {
            proximityThreshold = intent1.getIntExtra("proximityThreshold", 0);
        ...
        } else if (intent1.getStringExtra("mode").equals("area")) {
            north = intent1.getDoubleExtra("areaNorth", 0.0);
            east = intent1.getDoubleExtra("areaEast", 0.0);
            west = intent1.getDoubleExtra("areaWest", 0.0);
            south = intent1.getDoubleExtra("areaSouth", 0.0);
            areaDivider1 = new AreaDivider(north, east,
                    south, west, intent1.getIntExtra("cellSize", 0));
            visited = new boolean[areaDivider1.getYCells()][areaDivider1.getXCells()];
        }
    ...
    private void setUpMap() {
        ...
        intent1 = getIntent();
        if (intent1.getStringExtra("mode").equals("area")) {
            areaDivider1.renderGrid(map);
        }
        map.getUiSettings().setIndoorLevelPickerEnabled(false);
        map.getUiSettings().setMapToolbarEnabled(false);

        // Use the provided placeMarker function to add a marker at every target's location
        // HINT: onCreate initializes the relevant arrays (targetLats, targetLongs, path) for you
        if (intent1.getStringExtra("mode").equals("target")) {
    ...
    public void updateLocation(final double latitude, final double longitude) {
        if (intent1.getStringExtra("mode").equals("target")) {
            proximityThreshold = intent1.getIntExtra("proximityThreshold", 0);
            int i = TargetVisitChecker.getTargetWithinRange(targetLats, targetLngs,
                    path, latitude, longitude, proximityThreshold);
            if (i != -1 && TargetVisitChecker.checkSnakeRule(targetLats, targetLngs, path, i)) {
                int j = TargetVisitChecker.visitTarget(path, i);
                if (j != -1) {
                    ...
                    if (j > 0) {
                        ...
                        com.google.android.gms.maps.model.LatLng targetFirst = new LatLng(
                                targetLats[path[j - 1]], targetLngs[path[j - 1]]);
                        com.google.android.gms.maps.model.LatLng targetSecond = new LatLng(
                                targetLats[i], targetLngs[i]);
                        addLine(targetFirst, targetSecond, PLAYER_COLOR);
                        //addLine(targetLats[path[j - 1]], targetLngs[path[j - 1]],
                        //        targetLats[i], targetLngs[i], PLAYER_COLOR);
                    }
                }
            }
        } else if (intent1.getStringExtra("mode").equals("area")) {
            if ((longitude >= west && longitude <= east)
                ...
                int xCoordinate = areaDivider1.getXCoordinate(currentLocation);
                int yCoordinate = areaDivider1.getYCoordinate(currentLocation);
                ...
                int yY = visited.length - 1 - yCoordinate;
                if (empty) {
                    first = true;
                } else {
                    if (!visited[yY][xCoordinate]) {
                        int xX = areaDivider1.getXCoordinate(lastVisit);
                        int yYY = areaDivider1.getYCoordinate(lastVisit);
                        if ((Math.abs(xX - xCoordinate) == 1 && yYY == yCoordinate)
                                || (Math.abs(yYY - yCoordinate)
                                == 1 && xX == xCoordinate)) {
                            adjacent = true;
                        }
                    }
                ...
                if (adjacent || first) {
                    LatLngBounds bound = areaDivider1.getCellBounds(xCoordinate, yCoordinate);
                    ...
                    polygon.add(southWest, southEast, northEast, northWest);
                    polygon.fillColor(PLAYER_COLOR);
                    map.addPolygon(polygon);
                    visited[yY][xCoordinate] = true;
                    lastVisit = new LatLng(latitude, longitude);
                }
            }
        }
    }
    /*
    PolygonOptions polygon = new PolygonOptions();
    polygon.add(northEast, northWest, southWest, southEast);
    polygon.fillColor(PLAYER_COLOR);
     */
     ...
     */
    @VisibleForTesting
    public void addLine(final com.google.android.gms.maps.model.LatLng start,
                         final com.google.android.gms.maps.model.LatLng end,
                         final int color) {
